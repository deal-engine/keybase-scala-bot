...
object ExampleActions {
  ...
  val exchAction = ("tasa", BotAction(args => None, queryExchangeRates))
  ...
  def queryExchangeRates(args: String, replyFunction: BotAction.ReplyFunction): Future[Unit] = {
    (for {
      ...
      _ <- replyFunction(s"Buscando cambios para $baseCurrency hacia estas monedas: $exchangeCurrencies, en esta fecha $exchangeDate")
      responseBody = quickRequest.get(uri"http://api.openrates.io/$exchangeDate?base=$baseCurrency&symbols=$exchangeCurrencies").send().body
